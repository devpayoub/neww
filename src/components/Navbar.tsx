...
import { Menu, X, ChevronRight, Code, Palette, Megaphone, GraduationCap, Home, Info, Briefcase, BookOpen, Phone, Calendar, Image as ImageIcon } from 'lucide-react';
import {
    Sheet,
    SheetContent,
    SheetHeader,
...
export default function Navbar() {
    ...
    const menuItems = [
        ...
        { name: "À propos", href: "/about", icon: <Info className="w-5 h-5" /> },
        {
            ...
            submenu: [
                { name: "Développement web", href: "/services/web-development", icon: <Code className="w-5 h-5" /> },
                { name: "Marketing digital", href: "/services/digital-marketing", icon: <Megaphone className="w-5 h-5" /> },
                { name: "Design", href: "/services/graphic-design", icon: <Palette className="w-5 h-5" /> },
    ...
    return (
        <div>
            {/* Bandeau supérieur - visible only when not scrolled */}
            <div className={`flex text-center justify-center items-center bg-[#1D1046] p-2 transition-all duration-300 ${scrolled ? 'opacity-0 h-0 p-0 overflow-hidden' : 'opacity-100'}`}>
            ...
            <div style={{ height: scrolled ? `${navHeight}px` : '0px' }} className="transition-all duration-300"></div>

            {/* Barre de navigation - fixed when scrolled */}
            <div 
                id="main-navbar"
                className={`${scrolled ? 'fixed top-0 left-0 right-0' : ''} bg-white p-2 px-4 md:px-10 z-50 transition-all duration-300 ${scrolled ? 'shadow-md' : 'shadow-sm'}`}
            >
                                        ...
                                        <div className="absolute top-full left-0 mt-2 w-64 bg-white rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
                                            {item.submenu.map((subItem) => (
                                                <Link
                                                    key={subItem.name}
                                                    href={subItem.href}
                                                    className="flex items-center gap-2 px-4 py-3 hover:bg-gray-50 text-gray-700 hover:text-[#1D1046] transition-all"
                                                >
                                                    {subItem.icon}
                                                    {subItem.name}
                                                </Link>
                    ...
                    <div className="hidden md:flex gap-4">
                        <Link href="/consultation">
                            <motion.div
                                whileHover={{ 
                                    scale: 1.05,
                                    boxShadow: "0 10px 15px -3px rgba(29, 16, 70, 0.3), 0 4px 6px -2px rgba(29, 16, 70, 0.2)"
                                }}
                                whileTap={{ scale: 0.95 }}
                                transition={{ type: "spring", stiffness: 400, damping: 10 }}
                            >
                                <Button className="text-white bg-[#1D1046] hover:bg-[#2d1a69] px-4 py-2 rounded-[20px] flex items-center gap-2 transition-colors duration-300">
                                    <motion.div
                        ...
                        <Link href="/consultation">
                            ...
                            >
                                <Button
                                    variant="outline"
                                    size="sm"
                                    className="text-white bg-[#1D1046] hover:bg-[#2d1a69] px-3 py-1 rounded-[20px] text-xs flex items-center gap-1 transition-colors duration-300"
                                >
